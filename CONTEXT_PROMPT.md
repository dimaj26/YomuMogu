...
- **Chat Session Persistence**: Automatic state serialization and saving to localStorage (profile-namespaced format `chat_state_${sessionId}`). Restores conversation, collected words, and progress state on page reload/navigation. Features blue "Продолжить практику" buttons on homepage and settings grid for active sessions.
