...
- **Interval Systems Registry**: Timing or interval constants (Grammar Leitner, fading furigana, fluency limits, daily resets, competency limits) must be imported from `src/core/intervals.ts`. Direct values must not be hardcoded in consumers. Changing these values requires Route B (architectural audit).
