...
- **Japanification Progression**: Multi-stage UI progression shifting interface text into Japanese as XP increases.
- **Granular UI FSRS Japanification**: Smart localization wrapper component (`<JpUI>`) and provider (`JpUIProvider`) driven by the `ts-fsrs` mathematical scheduler. Dynamically translates UI elements (up to 1 new word per session), plays gold pulse animations on new translations, provides hover translation tooltips, and supports interactive FSRS assessments ("Забыл" / "Знаю") directly in the UI.
