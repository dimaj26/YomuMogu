...
- [ ] **T001** Fix dead graphify hooks — `.claude/settings.json`
...
- [ ] **T002** Close spec-sync guard loophole — `scripts/spec_sync_guard.py`
...
- [ ] **T003** Route the paid semantic pass through the subscription — `scripts/graph_doc_sync.py`
...
- [ ] **T004 [P]** Compact permission allow-list — `.claude/settings.local.json`
...
- [ ] **T005 [P]** Consolidate agent memory — `C:\Users\user\.claude\projects\C--YomuMogu\memory\`
...
- [ ] **T006 [P]** De-stale CLAUDE.md pointer — `CLAUDE.md`
...
- [ ] **T007 [P]** Snapshot retention — `.husky/pre-push`
...
- [ ] **T008 [P]** Constitution wording — `.specify/memory/constitution.md`
...
- [ ] **T009** Full verification
  1. `npx vitest run` — suite green.
  2. `npx lint-staged` on a scratch staged change or `node_modules/.bin/eslint .` — clean.
  3. Re-run T001/T002 acceptance one-liners.
  4. Commit(s) with `chore(tooling):`/`fix(tooling):` prefixes; **do not push** (explicit user approval required).
