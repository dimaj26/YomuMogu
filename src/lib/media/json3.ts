...
// Минимальные типы формата YouTube timedtext JSON3
interface Json3Seg {
  utf8: string;
  tOffsetMs?: number;
}
 ...
 */
export function parseJson3ToSegments(data: Json3Data): SubtitleSegment[] {
  const segments: SubtitleSegment[] = [];
  if (data && Array.isArray(data.events)) {
    for (const ev of data.events) {
      if (!ev.segs) continue;

      const text = ev.segs.map((s: Json3Seg) => s.utf8).join('').trim();
      if (!text) continue;

      // Извлекаем пословные тайминги с относительным сдвигом в миллисекундах
      const words = ev.segs.map((s: Json3Seg) => ({
        text: s.utf8,
